...
;Variables
  ...
  !define PRODUCT_NAME "Electrum for CPUchain"
  !define PRODUCT_WEB_SITE "https://github.com/cpuchain/electrum-cpu"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-cpu-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-cpu\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-cpu\*.*"
  File "c:\electrum-cpu\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe" 0


  ;Links cpuchain: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\cpuchain" "" "URL:cpuchain Protocol"
  WriteRegStr HKCU "Software\Classes\cpuchain" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\cpuchain" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\cpuchain\shell\open\command" "" "$\"$INSTDIR\electrum-cpu-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\cpuchain"
