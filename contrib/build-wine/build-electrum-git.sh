...
for i in ./locale/*; do
    dir=$WINEPREFIX/drive_c/electrum-cpu/electrum/$i/LC_MESSAGES
    mkdir -p $dir
...
info "Pip installing Electrum-CPU. This might take a long time if the project folder is large."
...
cd dist
mv electrum-cpu-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
